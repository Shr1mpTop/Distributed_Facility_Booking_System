...
| 功能 | 描述 | 操作类型 | 支持情况 |
|-----|------|---------|---------|
| 🔍 查询可用性 | 查看设施的可用时间段 | 幂等 | ✅ 所有客户端 |
| 📅 预订设施 | 预订特定时间段 | 非幂等 | ✅ 所有客户端 |
| ✏️ 修改预订 | 通过时间偏移修改预订 | 非幂等 | ✅ 所有客户端 |
| 👁️ **监控设施** | **实时接收设施可用性更新** | **回调通知** | ✅ **Python 客户端** |
| ⏱️ 获取最后预订时间 | 查询设施最后预订的结束时间 | 幂等 | ✅ 所有客户端 |
| ➕ 延长预订 | 延长现有预订的时长 | 非幂等 | ✅ 所有客户端 |

### 🆕 监控功能亮点

- **客户端注册**: 提供设施名称和监控时长，服务器记录客户端 IP 和端口
- **回调通知**: 监控期间任何预订/修改/扩展操作，服务器自动推送更新
- **超时终止**: 监控时长到期后自动停止，服务器清理客户端记录
- **客户端限制**: 监控期间用户无法输入新请求（符合项目要求）
- **多客户端支持**: 支持多个客户端并发监控同一设施
