...
ByteBuffer::ByteBuffer(const uint8_t *data, size_t len) : read_pos(0)
{
...
{
    if (read_pos + sizeof(uint16_t) > buffer.size())
    {
        throw std::runtime_error("Buffer underflow");
    }
    uint16_t net_val;
    std::memcpy(&net_val, &buffer[read_pos], sizeof(uint16_t));
...
{
    if (read_pos + sizeof(uint32_t) > buffer.size())
    {
        throw std::runtime_error("Buffer underflow");
    }
    uint32_t net_val;
    std::memcpy(&net_val, &buffer[read_pos], sizeof(uint32_t));
